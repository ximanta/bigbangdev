...
async def developer_node(state: GraphState) -> GraphState:
    ...
    prompt_text = (
        ...
        "=== UI, STYLING, AND INTERACTIVITY RULES ===\n"
        "- MUST use 'lucide-react' for ALL icons (e.g. <Search />, <Heart />, <Menu />) instead of emojis or SVG code. NEVER leave icons out if requested.\n"
        "- NEVER write dummy SVG components (e.g., const Globe = () => <svg...>) for icons. Always use imports from 'lucide-react'.\n"
        "- PREVENT ReferenceErrors: Ensure you never use a component or variable before it is declared. Remember that 'const' and 'let' are not hoisted.\n"
        "- All buttons and interactive elements MUST be clearly styled (padding, border-radius, background-color) and MUST have :hover and :active states in CSS.\n"
        "- ROUTING & NAVIGATION: If an element is meant to navigate to another page, you MUST use the `<Link>` component from `react-router-dom` or the `useNavigate` hook. Do NOT pass a `to` prop to a standard HTML `<button>` and expect it to work.\n"
        "- Ensure that buttons actually implement basic onClick handlers (even if it just toggles a state or sets a dummy value) so the UI doesn't feel broken.\n"
        "- NEVER generate text like 'Not available in this demo', 'Placeholder', or 'Coming soon'. The app MUST feel like a complete, fully functional product.\n"
        "- Design should feel premium, responsive, and complete. Avoid lazy placeholder text where possible.\n"
